...
const MainHeaderTop = () => {
  ...
  return (
    <div className="grid min-sm:grid-cols-3 max-sm:grid-cols-2 sm:items-center items-end pt-4 min-sm:pb-8">
      <div className='justify-self-start max-sm:order-1 min-sm:order-1'>
        <Link to="/">
          <img src={Logo} alt="logo" />
        </Link>
      </div>
      <div className='justify-self-center max-sm:order-3 min-sm:order-2 max-sm:col-span-2 max-sm:my-2'>
      ...
      <div className='justify-self-end max-sm:order-2 min-sm:order-3'>
        {isAuthenticated ? (
          <div className="flex gap-2 items-center relative hover:cursor-pointer">
            ...
            >
              {user.role === 'member' && (
                <NavLink to={user.isAccountSetting ? '/mypage' : '/account_setting'} state={{returnUrl:'/mypage'}}>
